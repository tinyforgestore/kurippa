...
/// Uses AtomicU64 (f64::to_bits encoding) so the CGEventTap callback never blocks on a Mutex.
/// u64::MAX is the sentinel meaning "no position recorded yet".
pub struct LastClickPos(pub Arc<AtomicU64>, pub Arc<AtomicU64>);
...
pub fn position_window(app: &AppHandle, window: &WebviewWindow) {
    ...
    let last_click: Option<(f64, f64)> = app
        ...
        .and_then(|s| {
            let x_bits = s.0.load(Ordering::Relaxed);
            let y_bits = s.1.load(Ordering::Relaxed);
            if x_bits == u64::MAX || y_bits == u64::MAX {
                None
            } else {
                Some((f64::from_bits(x_bits), f64::from_bits(y_bits)))
            }
        });
